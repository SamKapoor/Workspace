...
public class DevComm {
	...
	public DevComm(Activity parentActivity, IUsbConnState usbConnState) {
		...
		serialport
				.setOnserialportDataReceivedOhter(new SerialPortDataReceivedOther() {
					@Override
					public void onDataReceivedListener(final byte[] buffer,
							int size) {
						// final String str=byteToString(buffer,size);
						// Log.i(TAG,"setOnserialportDataReceivedOhter,onDataReceivedListener:"+str);
	...
	public boolean OpenComm() {
		...
		if (serialport.OpenSerialPort(SerialPortClass.choosed_serial,
				SerialPortClass.choosed_buad)) {
	...
	public int Run_TestConnection() {
		...
		String str=SerialPortHelper.byte2HexStr(bt);
